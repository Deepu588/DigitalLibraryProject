...
<%
        List<User> l = (List) request.getAttribute("allUsers");
System.out.println(l);
        if (l == null ) {
    %>
        <h3 class="display-1 text-center text-light my-5">No Account are created till now</h3>
    <%
        } else {
        	int c=0;
    %>
       ...
       <table class="table table-bordered  border border-light my-5">
            <thead>
            <tr><td colspan="9" class="text-lg text-center text-light display-6">VIEW USERS </td></tr></thead>
                           <tbody>
                <tr class="text-center text-light text-lg">
                    <th>SNO</th>
                    <th>UserName</th>
                    <th>Age</th>
<th>City</th><th>Contact No</th><th>Email</th><th>Password</th>
                ...
                %>
                    <tr class="text-center text-light text-md">
                        ...
                        <td><%=u.getEmail() %></td>
                        <td><%=u.getPassword().hashCode() %></td>
                        
                        <td><a href="mailto:<%=u.getEmail() %>" target="_blank" class="btn btn-success text-light">Contact</a></td>
                        <td><form method="post" action="deleteUser"><input type="hidden" name="uid" value=<%=u.getUserId() %>><button class="btn btn-danger"  type="submit"><i class="bi bi-trash"></i>Delete</button></form></td>
                    </tr>
                <%
                    }
                %>
